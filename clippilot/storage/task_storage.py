...
class TaskStorage:
    ...
    def create_task_paths(self, task_id: str, original_file_name: str) -> TaskPaths:
        ...
        for directory in (
            ...
            task_paths.understanding_frames_dir,
            task_paths.understanding_windows_dir,
            task_paths.window_requests_dir,
            task_paths.window_responses_dir,
            task_paths.window_results_dir,
            task_paths.refinement_dir,
    ...
    def save_sampled_frames(self, sampled_frames: SampledFramesResult | BaseModel | dict, task_paths: TaskPaths) -> Path:
        """Persist sampled frame metadata used as visual input for video understanding."""

        return self.save_model(sampled_frames, task_paths.sampled_frames_path)

    def save_content_analysis(self, content_analysis: BaseModel | dict, task_paths: TaskPaths) -> Path:
        """Persist content-analysis output generated by the video-understanding stage."""

        return self.save_model(content_analysis, task_paths.content_analysis_path)

    def save_llm_candidates(self, llm_candidates: BaseModel | dict, task_paths: TaskPaths) -> Path:
        """Persist transcript-backed LLM highlight candidates for downstream planning."""

        return self.save_model(llm_candidates, task_paths.llm_candidates_path)

    def save_video_understanding_request(self, request_payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        """Persist the prompt/request payload prepared for the video-understanding model."""

        return self.save_model(request_payload, task_paths.video_understanding_request_path)

    def save_video_understanding_response_raw(self, response_payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        """Persist the raw response payload returned by the video-understanding stage."""

        return self.save_model(response_payload, task_paths.video_understanding_response_raw_path)

    def save_video_windows(self, payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        return self.save_model(payload, task_paths.video_windows_path)

    def save_window_request(self, window_id: str, payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        return self.save_model(payload, task_paths.window_requests_dir / f"{window_id}.json")

    def save_window_response(self, window_id: str, payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        return self.save_model(payload, task_paths.window_responses_dir / f"{window_id}.json")

    def save_window_result(self, window_id: str, payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        return self.save_model(payload, task_paths.window_results_dir / f"{window_id}.json")

    def save_global_understanding_request(self, payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        return self.save_model(payload, task_paths.global_understanding_request_path)

    def save_global_understanding_response(self, payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        return self.save_model(payload, task_paths.global_understanding_response_path)

    def save_global_understanding(self, payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        return self.save_model(payload, task_paths.global_understanding_path)

    def save_refinement_request(self, group_id: str, payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        return self.save_model(payload, task_paths.refinement_dir / f"{group_id}_request.json")

    def save_refinement_response(self, group_id: str, payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        return self.save_model(payload, task_paths.refinement_dir / f"{group_id}_response.json")

    def save_refined_candidates(self, payload: BaseModel | dict, task_paths: TaskPaths) -> Path:
        return self.save_model(payload, task_paths.refined_candidates_path)
