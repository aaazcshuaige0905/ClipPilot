...
def _build_test_settings(root: Path) -> AppSettings:
    """Create isolated settings for revision-agent tests."""
...
def _build_user_request() -> UserRequest:
    """Create a reusable request fixture for revision-agent tests."""
...
def _build_video_info() -> VideoInfo:
    """Create reusable source video metadata for revision tests."""
...
def _build_llm_candidates() -> list[LLMHighlightCandidate]:
    """Create transcript-backed highlight candidates for revision tests."""
    ...
    return [
        ...
        LLMHighlightCandidate(
            ...
            transcript_unit_ids=["unit_002"],
            source_start=3.3,
            source_end=7.0,
            duration=3.7,
            title="Support proof",
            summary="Keep a quick supporting proof.",
            transcript_excerpt="Supporting proof with numbers.",
            highlight_type="supporting_point",
            semantic_role="supporting_point",
            reason="Adds evidence without slowing pacing.",
            scores=HighlightScoreBreakdown(
                overall=0.84,
                hook=0.3,
                emotion=0.41,
...
def _build_timeline() -> VideoTimeline:
    """Create a semantic timeline for revision-agent tests."""
    ...
    return VideoTimeline(
        ...
        segments=[
            ...
            TimelineSegment(
                ...
                duration=3.7,
                title="Proof",
                summary="Supporting proof with numbers.",
                transcript_excerpt="Supporting proof with numbers.",
...
def _build_retrieved_context() -> RetrievedContext:
    """Create retrieved rules for revision-agent tests."""
    ...
    return HighlightCandidatesResult(
        ...
        candidates=[
            ...
            HighlightCandidate(
                ...
                text="Supporting proof with numbers.",
                score=0.84,
                reason="Compact supporting evidence.",
            ),
            HighlightCandidate(
                start=8.0,
                end=11.5,
...
def test_revise_editing_plan_applies_feedback_and_persists_new_version() -> None:
    """Ensure revision feedback updates planner memory and writes a new plan version."""

    settings = _build_test_settings(_workspace_root("revision_agent"))
    ...
    revised_plan = revise_editing_plan(
        task_id="task123",
        feedback_text='改成20秒，保留 cand_ending，不要 cand_hook，并且保留 "Final takeaway and CTA."',
        settings=settings,
    )
    loaded_memory = storage.load_planner_memory("task123")
    loaded_project_state = storage.load_project_state("task123")
    loaded_task_result = storage.load_task_result("task123")
    loaded_plan = storage.load_editing_plan("task123")

    assert revised_plan.plan_version == 2
    assert loaded_plan.plan_version == 2
    assert loaded_memory.current_plan_version == 2
    assert loaded_memory.active_constraints["target_duration"] == 20
    assert "cand_ending" in loaded_memory.locked_elements.must_keep_candidate_ids
    assert "cand_hook" in loaded_memory.locked_elements.forbidden_candidate_ids
    assert "Final takeaway and CTA." in loaded_memory.locked_elements.must_keep_quotes
    assert loaded_memory.instruction_history[-1].turn_id == "revision_02"
    assert loaded_task_result.status == "processing"
    assert loaded_task_result.stage == "editing_plan_generated"
    assert loaded_task_result.final_video_path is None
    assert loaded_project_state.user_request.target_duration == 20
    assert loaded_project_state.qc_report is None
    assert task_paths.plan_versions_dir.joinpath("editing_plan_v02.json").exists()
    assert any("Revision feedback applied" in note for note in revised_plan.editing_notes)
